configure_operations_basics() {
  ...
  write_text "$(target_path /usr/local/sbin/nas-notify)" \
    "#!/usr/bin/env bash"$'\n'\
"set -euo pipefail"$'\n'\
"# Placeholder notification helper generated by NAS bootstrap."$'\n'\
"# It intentionally does not deliver Discord or Healthchecks.io alerts yet."$'\n'\
"# Future alert wiring should read private webhook URLs from a local ignored env file."$'\n'\
"printf '[nas-notify placeholder] %s\n' \"\${*:-notification requested, but no delivery backend is configured}\" >&2"$'\n'
